...
export default function Sidebar({
  user, language, isOpen, history, showSettings, openMenuId, shariaEnabled, t,
  onToggle, onSignOut, onToggleSettings, onLanguageToggle, onShariaToggle,
            ...
            {showSettings && (
              <div className="mt-2 bg-[#0F0D0A] border border-[#1A1610] rounded-xl shadow-2xl p-2">
                ...
                <div
                  ...
                  className="flex items-center justify-between px-3 py-2.5 hover:bg-[#13110E] rounded-lg transition-colors cursor-pointer active:scale-[0.98]"
                >
                  <span className="text-xs text-[#D4C8A0]">{t.shariaToggle || "Şer'i Uyum"}</span>
                  <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded border ${
                    shariaEnabled
                      ? "text-green-400 bg-green-500/10 border-green-500/20"
                      : "text-[#605030] bg-[#13110E] border-[#2A2520]"
                  }`}>
                    {shariaEnabled ? "ON" : "OFF"}
                  </span>
                </div>
